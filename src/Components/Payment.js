...
const Payment = ({ onSubmit }) => {
  const [state, setState] = useState({ chname: '', cnumber: '', expiry: '', cvc: '', focused: '', issuer: '' });
  ...
  const handleInputChange = ({ target }) => {
    let { name, value } = target;
    if (name === 'cnumber') value = formatCreditCardNumber(value); // Change made here
    else if (name === 'expiry') value = formatExpirationDate(value);
    else if (name === 'cvc') value = formatCVC(value); // Change made here
    setState({ ...state, [name]: value });
  };
    ...
    <div key='Payment'>
      <div className='App-payment container border pb-3' style={{ width: '40vw' }}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className='form-group'>
            <small>CVV:</small>
            <input type='tel' name='cvc' className='form-control' placeholder='CVV' pattern='\d{3,4}' required value={cvc} onChange={handleInputChange} onFocus={handleInputFocus} />
          </div>
          <input type='hidden' name='issuer' value={issuer} />
          <div className='form-actions'>
            <button className='btn btn-success' style={{ justifyContent: 'center', margin: 'auto', display: 'flex' }}>Pay</button>
